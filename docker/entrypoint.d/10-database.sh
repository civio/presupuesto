...
for i in $(seq 1 $END); do
	...
	if PGPASSWORD=$DATABASE_PASSWORD psql --username=$DATABASE_USER --host=$DATABASE_HOST --port=$DATABASE_PORT --dbname=postgres --command='SELECT now()'; then
...
PGPASSWORD=${DATABASE_PASSWORD} \
  psql --username=${DATABASE_USER} --host=${DATABASE_HOST} --port=${DATABASE_PORT} --dbname=postgres \
